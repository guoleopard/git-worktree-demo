...
分析使用者需求，判断是否适合以 `git worktree` 拆分成多个 feature branch 平行开发，提供建议方案并执行。
...
针对每个拆分出来的 feature，根据使用者的原始需求和专案现况，**自动推导**出完整的 Spec 内容（不是丢空白范本）。
...
- 取 repo 目录名作为 `<project-name>` 前缀，避免与其他专案混淆
